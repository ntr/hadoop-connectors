...
public class GcsDelegationTokens extends AbstractService {
  ...
  private GCSTokenInstantiationStrategy tokenInstantiationStrategy = GCSTokenInstantiationStrategy.INSTANCE_PER_SERVICE;
  private static Map<String, Token<DelegationTokenIdentifier>> sharedTokens = new ConcurrentHashMap<>();
  ...
  public Token<DelegationTokenIdentifier> getBoundOrNewDT(String renewer) throws IOException {
    ...
    if(this.tokenInstantiationStrategy == GCSTokenInstantiationStrategy.SHARED)
    {
      synchronized (sharedTokens){
        if(sharedTokens.containsKey(renewer))
        {
          ...
          Token<DelegationTokenIdentifier> token = tokenBinding.createDelegationToken(renewer, getStats());
